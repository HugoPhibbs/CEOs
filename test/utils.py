...
def stream_ceos_hash1(exact_kNN, X, Q, k=10, D=2**10, top_m=100, iProbe=0, probed_vectors=20, probed_points=50, n_repeats=1, n_threads=8, seed=-1, verbose=True, centering=False):
    ...
    t2 = timeit.default_timer()
    print('streamCEOs-Hash index time (s): {}'.format(t2 - t1))
    ...
    for i in range(1):
        ...
        t1 = timeit.default_timer()
        approx_kNN, approx_Dist = index.search(Q, k, verbose)  # search
        print("\tstreamCEOs-Hash query time (s): {}".format(timeit.default_timer() - t1))
        print("\tstreamCEOs-Hash accuracy: ", getAcc(exact_kNN, approx_kNN))

def stream_ceos_stream_hash1(X, Q, k=10, D = 2**10, top_m=100, iProbe=2, probed_vectors=20, n_repeats=1, n_threads=8, seed=-1, verbose=True, centering=False):
    ...
    index = CEOs.streamCEOs(d)
    index.setIndexParam(D, n_repeats, top_m, iProbe, n_threads, seed)
    index.build1(X[0 : n_size, :])
    print('streamCEOs-hash1 index time: {}'.format(timeit.default_timer() - t1))

    index.n_probed_vectors = probed_vectors
    exact_kNN = faissBF(X[0: n_size, :], Q, k, n_threads)

    t1 = timeit.default_timer()
    approx_kNN, approx_dist = index.search1(Q, k, verbose)  # search
    print('\tstreamCEOs-hash1 query time: {}'.format(timeit.default_timer() - t1))
    print("\tstreamCEOs-hash1 Accuracy: ", getAcc(exact_kNN, approx_kNN))
    ...
    for i in range(9):
        ...
        # add & remove items
        t1 = timeit.default_timer()
        index.update1(new_X, n_size)  # X must have d x n
        print('\tstreamCEOs-hash1 update index time: {}'.format(timeit.default_timer() - t1))
...
def stream_ceos_stream_hash2(X, Q, k=10, D = 2**10, top_m=100, iProbe=2, probed_vectors=20, n_repeats=1, n_threads=8, seed=-1, verbose=True, centering=False):
    ...
    index = CEOs.streamCEOs(d)
    index.setIndexParam(D, n_repeats, top_m, iProbe, n_threads, seed)
    index.build2(X[0 : n_size, :])
    print('streamCEOs-hash2 index time: {}'.format(timeit.default_timer() - t1))

    index.n_probed_vectors = probed_vectors
    exact_kNN = faissBF(X[0: n_size, :], Q, k, n_threads)

    t1 = timeit.default_timer()
    approx_kNN, approx_dist = index.search2(Q, k, verbose)  # search
    print('\tstreamCEOs-hash2 query time: {}'.format(timeit.default_timer() - t1))
    print("\tstreamCEOs-hash2 Accuracy: ", getAcc(exact_kNN, approx_kNN))
    ...
    for i in range(9):
        ...
        # add & remove items
        t1 = timeit.default_timer()
        index.update2(new_X, n_size)  # X must have d x n
        print('\tstreamCEOs-hash2 update index time: {}'.format(timeit.default_timer() - t1))
